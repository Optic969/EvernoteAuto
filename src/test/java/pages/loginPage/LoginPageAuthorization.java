...
public class LoginPageAuthorization {
    ...
    public void loginWithInvalidCredentials() {
        $x(EMAIL_FIELD).sendKeys(TestData.INVALID_EMAIL);
        $x(CONTINUE_BUTTON).click();
        $x(PASSWORD_FIELD).shouldBe(visible).sendKeys(TestData.INVALID_PASSWORD);
        $x(CONTINUE_BUTTON).click();
    ...
    public void loginWithValidCredentials() {
        open(TestData.LOG_IN_URL);
        $x(EMAIL_FIELD).shouldBe(visible);
        $x(EMAIL_FIELD).click();
        $x(EMAIL_FIELD).setValue(TestData.VALID_EMAIL);
        $x(CONTINUE_BUTTON).shouldBe(enabled).click();
        $x(PASSWORD_FIELD).click();
        $x(PASSWORD_FIELD).setValue(TestData.VALID_PASSWORD);
        $x(CONTINUE_BUTTON).shouldBe(enabled).click();
        webdriver().shouldHave(url(TestData.CLIENT_WEB_URL), Duration.ofSeconds(10));
    }
}